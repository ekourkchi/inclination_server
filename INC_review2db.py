...
\n
 - Ingesting the Input catalog (for r eviwing) into the Data-Base 
 - The input list has at least one columns, i.e. "pgc" as the header

 - How to run: 
 
    $ python %prog -i [<input_list> , e.g. pgc_SDSS_calib_review.lst] -t [<table_name> , e.g. SDSS_calib_review]
    $ python %prog -h 
...
data = []
for pgc in pgcs: 
    data.append((pgc, 0, 0))   # default values: dPA=0 and zoom=1
    
cur=db.cursor()
sql_command = "INSERT INTO "+table_name+" (pgcID, seen, selected) VALUES (%s, %s, %s)"
